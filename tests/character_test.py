...
class TestCharacterMovement:
    ...
    def test_move_not_blocked_by_different_character_at_different_position(
        self, mock_grid, character
    ):
...
class TestCharacterActions:
    ...
    def test_process_signals_clears_signals(self, character):
        ...
        old_time = (
            pygame.time.get_ticks() - 11000
        )  # 11 seconds ago (SIGNAL_MEMORY_TIME is 10000)
    ...
    def test_process_signals_returns_list_not_none(self, character):
        ...
        character.signals = [
            {"type": "test", "data": {}, "time": pygame.time.get_ticks()}
        ]
        character.process_signals()
    ...
    def test_process_signals_less_than_comparison(self, character):
        ...
        character.signals = [
            {"type": "boundary", "data": {}, "time": exact_boundary_time}
        ]
        ...
        assert len(character.signals) == 0, (
            "Signal at exact boundary should be removed with < operator"
        )
    ...
    def test_process_signals_just_before_boundary(self, character):
        ...
        just_under_boundary = (
            pygame.time.get_ticks() - character.SIGNAL_MEMORY_TIME + 100
        )  # 100ms before boundary
        character.signals = [
            {"type": "recent", "data": {}, "time": just_under_boundary}
        ]
...
class TestCharacterObservation:
    ...
    def test_get_observation_strict_boundary_check(self, character):
        ...
        for i in range(1, 10):
            for j in range(1, 10):
                assert observation[-1][i, j] == 7, (
                    f"Cell [{i},{j}] should be 7, got {observation[-1][i, j]}"
                )
    ...
    def test_get_observation_local_x_start_must_be_int_not_none(self, character):
        ...
        assert observation[-1][5, 4] == 100  # global[10, 0]
        assert observation[-1][5, 5] == 101  # global[10, 1]
        logging.debug(observation[-1])


class TestCharacterSurrounding:
    """Tests for getting surrounding characters"""

    def test_send_signal_default_broadcast_range_is_3_coverage(
        self, mock_grid, character
    ):
...
class TestCharacterBoundaryObservation:
    ...
    def test_get_observation_boundary_condition_uses_greater_than_not_gte(
        self, character
    ):
...
class TestCharacterDrawing:
    ...
    def test_draw_with_offset_y_adds_correctly(self, mock_rect, character):
        ...
        assert (
            actual_y != character.y * CELL_SIZE - offset_y
        )  # Would be wrong with - operator
