...
class Human(Character):
    ...
    def get_target_vector(self) -> np.ndarray:
            ...
            if isinstance(char, (Zombie, Infected)):
                ...
                if dist_sq < (self.THREAT_DETECTION_RANGE**2):
                    if dist_sq < min_threat_dist:
                        ...
                        closest_threat_pos: tuple[Any, Any] = (char.x, char.y)
        ...
        soldier_vec: list[float] = [0.0, 0.0]
    ...
    def act(self) -> float | int:
        ...
        if min_real_dist < float("inf"):
            ...
            if min_real_dist < SAFE_DISTANCE:
                panic_penalty: float = (SAFE_DISTANCE - min_real_dist) * 0.2
            ...
            self.prev_min_dist: float = min_real_dist
        else:
            self.prev_min_dist = float("inf")
    ...
    def broadcast_threat_info(self, threats) -> None:
    ...
    def get_infected(self) -> None:
        ...
        self.is_alive = False
        logging.info(f"Człowiek {id(self)} został zarażony! Kara -10")
        # Wołamy o pomoc medyka!
        self.broadcast_help_request()

    def broadcast_help_request(self) -> None:
