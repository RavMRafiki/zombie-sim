...
class Soldier(Character):
    ...
    def get_target_vector(self) -> np.ndarray:
        ...
        time_since_shot: int = current_time - self.last_kill_time
        weapon_status = 1.0
        if time_since_shot < self.KILL_COOLDOWN:
            weapon_status: float = time_since_shot / self.KILL_COOLDOWN
    ...
    def act(self) -> float | int:
            ...
            for character in self.grid.characters:
                if isinstance(character, (Zombie, Infected)):
                    ...
                    if distance <= self.KILL_RANGE:
                        ...
                        self.last_kill_time: int = current_time
        ...
        if not target_pos:
            for signal in self.signals:
                if signal["type"] == "threat_alert":
                    s_pos = signal["data"]["source_pos"]
                    d: float = math.sqrt((self.x - s_pos[0]) ** 2 + (self.y - s_pos[1]) ** 2)
                    if d < min_dist:
                        min_dist: float = d
                        target_pos = s_pos
        ...
        if target_pos:
            ...
            is_weapon_ready: bool = (current_time - self.last_kill_time) > (
                self.KILL_COOLDOWN * 0.8
            )
    ...
    def broadcast_kill(self, kill_pos) -> None:
