...
class Infected(Character):
    ...
    previous_type: str = "Human"
    color = (255, 165, 0)
    move_speed = int(MOVE_INTERVAL * 0.75)
    TRANSFORMATION_TIME = 10000
    BROADCAST_RANGE = 5.0

    def __init__(self, x, y, grid=None, previous_type=None) -> None:
        super().__init__(x, y, grid)
        self.infection_start_time: int = pygame.time.get_ticks()
        if previous_type:
    ...
    def act(self) -> None:
        ...
        time_infected: int = current_time - self.infection_start_time
    ...
    def broadcast_infected_status(self) -> None:
        """Broadcast infected status to nearby characters"""

        time_left: int = self.TRANSFORMATION_TIME - (
            pygame.time.get_ticks() - self.infection_start_time
        )

        self.send_signal(
            signal_type="got infected",
            broadcast_range=self.BROADCAST_RANGE,
            data={"time_until_zombie": time_left},
        )

    def transform_to_zombie(self) -> None:
        """Convert this infected to zombie"""
        if self.grid:
            from zombie import Zombie

            idx = self.grid.characters.index(self)
            zombie: Zombie = Zombie(self.x, self.y, self.grid)
