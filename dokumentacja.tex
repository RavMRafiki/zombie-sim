...
\begin{document}
	...
	System ``Zombie Outbreak Simulation'' jest symulacją wieloagentową opracowywaną w środowisku gry 2D. Model systemu stanowi szachownica o wymiarach 256x256 komórek, na której poruszają się pięć typów autonomicznych agentów posiadających różne cele i zachowania. Każdy agent podejmuje decyzje w oparciu o obserwacje otoczenia i stanu wewnętrznego, stanowiąc część zamodelowanego ekosystemu zmagan ludzi zombie.
	
	\subsection{Uzasadnienie w Kontekście Budowanego Systemu}
	Model wieloagentowy (MAS - Multi-Agent System) pozwala na symulowanie złożonych interakcji między jednostkami o różnych celach strategicznych. W kontekście symulacji epidemiologicznej, taki system umożliwia:
	\begin{itemize}
		...
		\item Testowanie algorytmów sztucznej inteligencji (Deep Q-Network) do sterowania agentami
	\end{itemize}
	
	\section{Opis Programu}
	
	\subsection{Opis dla Klienta}
	System symuluje wybuch epidemii zombie w zamkniętym środowisku. Użytkownik obserwuje w czasie rzeczywistym:
	\begin{itemize}
		...
		\item Rozprzestrzenianie się infekcji i jej hamowanie poprzez działania medyków i żołnierzy
	...
	\begin{itemize}
		...
		\item Liczba postaci początkowych: 40 zombie, 60 ludzi, 0 zarażonych, 0 medyków, 0 żołnierzy
	\end{itemize}
	...
	Na górze okna wyświetlany jest licznik aktualnych populacji każdego typu postaci w formacie:
	\texttt{Zombies: X | Humans: Y | Infected: Z | Medics: A | Soldiers: B}
	...
	Stan systemu w chwili czasu $t$ opisany jest jako:
	$$S(t) = \{A_1(t), A_2(t), \ldots, A_n(t)\}$$
	gdzie $A_i(t) = (x_i(t), y_i(t), type_i, state_i(t))$ reprezentuje $i$-tego agenta z pozycją $(x_i, y_i) \in \{0, \ldots, 255\}^2$, typem i stanem wewnętrznym.
	
	\subsubsection{Model Obserwacji Agenta}
	Każdy agent obserwuje lokalne otoczenie poprzez macierz obserwacji $O_i(t) \in \mathbb{R}^{4 \times 11 \times 11}$, gdzie:
	\begin{itemize}
		...
		\item Wartości: -1.0 dla wrogów (zombie/zarażeni), 0.5 dla sojuszników (ludzie/medycy/żołnierze), 0 dla pustej przestrzeni
	\end{itemize}
	
	\subsubsection{Dynamika Ruchu}
	Ruch każdego agenta jest dyskretny, możliwe akcje: $a \in \{0,1,2,3,4\}$
	$$\begin{cases}
	...
	Nowa pozycja obliczana jest jako:
	$$x'_i = \text{clip}(x_i + dx, 0, 255), \quad y'_i = \text{clip}(y_i + dy, 0, 255)$$
	
	Ruch jest wykonywany tylko gdy nowa pozycja nie jest zajęta przez innego agenta.
	...
	Medycy leczą zarażonych przywracając ich do poprzedniego stanu, jeśli dystans jest mniejszy niż 1.5:
	$$d_{healing} = \sqrt{(x_m - x_{inf})^2 + (y_m - y_{inf})^2} \leq 1.5$$
	...
	Zombie otrzymują nagrodę za zbliżanie się do i zarażanie ludzi:
	$$r_z(t) = \begin{cases}
	...
	DQN przyjmuje obraz 4x11x11 jako wejście i zwraca Q-wartości dla każdej akcji (5 akcji):
	$$Q(s,a) \approx q_\theta(s)$$
	...
	Algorytm Deep Q-Learning z funkcją straty:
	...
	\begin{algorithm}[H]
		...
		Oblicz minimalny dystans do najbliższego człowieka\;
		\If{$d_{min} < 5.0$}{
	...
	System symulacji jest całkowicie offline - nie komunikuje się z siecią. Potencjalne zagrożenia wewnętrzne:
	\begin{itemize}
	...
	\subsubsection{Opis}
	Klasa \texttt{Character} jest abstrakcyjną klasą bazową dla wszystkich typów agentów. Definiuje wspólne zachowania i atrybuty.
...
def get_observation(self):
	...
	\subsubsection{Opis}
	Zombie są głównymi antagonistami. Aktywnie szukają i zarażają ludzi, otrzymując nagrody za zbliżanie się i zarażanie.
...
def act(self):
	...
	\subsubsection{Opis}
	Ludzie są defensywni. Unikają zombie, rozglaszają ostrzeżenia i współpracują z innymi postaciami.
	
	\subsubsection{Zachowanie act()}
	Ludzie skanują otoczenie w poszukiwaniu zombie/zarażonych. Jeśli zostaną wykryte, rozglaszają informację o zagrożeniu w promieniu 7.0 komórek.
	
	\subsection{Klasa Grid}
	
	\subsubsection{Opis}
	Klasa \texttt{Grid} zarządza całą symulacją. Przechowuje wszystkich agentów, obsługuje aktualizacje i renderowanie.
...
def main():
	...
	Projekt zawiera kompleksowe testy jednostkowe w pliku \texttt{tests/character\_test.py} obejmujące:
	\begin{enumerate}
...
def test_move_up(character):
	...
	\begin{itemize}
		\item Liczba testów: 45+
		\item Testy przechodzące: 40+
		\item Pokrycie kodu: ~75\% klasy Character
	\end{itemize}
	...
	Testy mutacyjne sprawdzają, czy zmiany w kodzie są wykrywane przez testy jednostkowe. System MutMut automatycznie wprowadza małe zmiany (mutacje) w kodzie i sprawdza, czy testy je wykrywają (killed) czy nie (survived).
	
	\subsubsection{Strategia Testowania}
	
	\begin{enumerate}
		\item Uruchomienie: \texttt{mutmut run --paths-to-mutate=character.py}
		\item Analiza wyników: \texttt{mutmut results}
		\item Wyświetlenie szczegółów: \texttt{mutmut show character.py}
	\end{enumerate}
...
if current_time - self.last_move_time > self.move_speed:
		...
		\item \textbf{Mutacja 43} - Zmiana zakresu pętli w obserwacji
		Status: \textbf{SURVIVED} (testy nie wykrywają - wymaga wzmocnienia)
	\end{itemize}
	...
	\begin{enumerate}
		\item \textbf{Test zarażenia} - Zombie znajduje się blisko człowieka, zarażenie powinno nastąpić
		\item \textbf{Test leczenia} - Medyk leczy zarażonego, przywracając typ
		\item \textbf{Test eliminacji} - Żołnierz eliminuje zombie
		\item \textbf{Test komunikacji} - Człowiek rozsyła ostrzeżenie innym
		\item \textbf{Test kolizji} - Dwa agenci nie mogą zajmować tej samej komórki
		\item \textbf{Test granic} - Agenci nie mogą wyjść poza siatkę
	\end{enumerate}
	
	\subsubsection{Metodologia Testowania}
	
	Każdy test funkcjonalny:
	\begin{enumerate}
		\item Inicjuje kontrolowaną konfigurację systemu
		\item Wywoływuje konkretne akcje
		\item Weryfikuje osiągnięcie oczekiwanego stanu
		\item Dokumentuje wynik (PASS/FAIL)
	\end{enumerate}
	...
	\subsection{Diagram Interakcji Systemu}
	
	\begin{verbatim}
	Grid ----+------+-------+-------+
	         |      |       |       |
	      Zombie  Human Infected Medic
	         |      |       |       |
	         +------+-------+-------+
	      (All inherit from Character)
	
	Communication:
	Zombie --(infects)--> Human --(broadcasts)--> Medic
	                         |
	                    Infected --(gets_healed)--> Original type
	                         |
	                       Zombie --(attacked_by)--> Soldier
	\end{verbatim}
	...
	- $n = 100$ agentów (40 zombie + 60 ludzi)
	- $O_{frame} = O(100^2) = 10,000$ operacji podstawowych na klatkę
	- FPS = 60 → $60 \times 10,000 = 600,000$ operacji/sekundę
	...
	Dla $n=100$: $M_{total} \approx 100 \times 4\text{ KB} + 500\text{ KB} + 2\text{ MB} \approx 2.9\text{ MB}$
	...
	Obecnie $O(n^2)$ jest nieuniknione ze względu na potrzebę sprawdzania wszystkich agentów. Możliwa optymalizacja:
	...
	\begin{itemize}
		...
		\item Złożoność ulepszona do: $O(n \cdot k)$ gdzie $k$ = średnia liczba agentów w sąsiadujących komórkach ($k \ll n$)
	\end{itemize}
...
def broadcast_threat_info(self, threats):
	...
	\begin{itemize}
		\item Architektura oparta na dziedziczeniu (Character) pozwala na łatwe dodawanie nowych typów agentów
